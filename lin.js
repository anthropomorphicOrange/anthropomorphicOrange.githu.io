 ...
 /*   timezone.innerHTML = 'America/Cocoa Beach';
    ...
    data.daily.forEach((day, idx) => {
      ...
      if(idx < 7){
        ...
        <div class="md:flex-row lg:flex-row flex-col md:items-start md:justify-start m-0  border-2 box border-slate-500 px-0 py-0 bg-pink-100/[.8] text-center text-stone-700 rounded-2xl" id="current-temp">
            <div data-category="current-temp" >
            <div class="py-1 px-1" data-category="day">${window.moment(day.dt * 1000).format('ddd')}</div>
           <div class="px-5 py-5"> <img src="https://openweathermap.org/img/wn/${day.weather[0].icon}.png"  alt="weather icon" class="w-icon"> </div>                                          
            <div class="py-1 px-8" data-category="night">Night: ${day.temp.night}&#176;</div>  
            <div class="py-1 px-8" data-category="day">Day: ${day.temp.day}&#176;</div>
        </div>
        </div>
