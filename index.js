...
function getWeatherData () {
    ...
    navigator.geolocation.getCurrentPosition((success) => {
          ...
          fetch(`https://api.openweathermap.org/data/3.0/onecall?lat=${32.0522}&lon=${118.2437}&exclude=hourly,minutely&units=imperial&appid=${API_KEY}`)
 ...
 /*   timezone.innerHTML = 'America/Cocoa Beach';
    ...
    data.daily.forEach((day, idx) => {
      ...
      if(idx < 7){
        ...
        <div class="flex space-x-14 flex-row items-center justify-center ml-20 mr-10 border-4 box border-slate-500 px-5 py-1 bg-pink-100/[.8] text-center text-stone-700 rounded-2xl" id="current-temp">
            <div data-category="current-temp" >
            <div class="py-0 pr-5 pl-5" data-category="day">${window.moment(day.dt * 1000).format('ddd')}</div>
            <img src="https://openweathermap.org/img/wn/${day.weather[0].icon}.png" alt="weather icon" class="w-icon">                                           
            <div class="py-1 pl-0 pr-5" data-category="night">Night ${day.temp.night}&#176;</div>  
            <div class="py-1 pl-0 pr-5" data-category="day">Day ${day.temp.day}&#176;</div>
        </div>
        </div>
